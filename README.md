...
EZZAIM Adam - 325748 
HOMINAL Anders - 297073
LAHLOU NABIL Malak - 329571
BOUDARKA Omar -  328645
...
To run our project you should:

1) First run the notebook `pre_processing.ipynb` to filter relevant data (to form edges and nodes).
2) Run the notebook `delay_modelling.ipynb` to get the historical data from istaden table for delays.
3) Finally, run the notebook `visual.ipynb` to get an overview of our results.

Our project contains `.py`files, these are used for utility functions. The `.pynb`notebooks are the one used for running the code.

The video for our project can be found here: https://drive.google.com/file/d/17a84fiTWjGXMuu8qyU_HffbWwRqjBNMN/view?usp=drivesdk
